...
export class ToolGenerator {
     ...
     */
    generateToolName(protocolName, messageName) {
     ...
     */
    generateToolDescription(spec, messageType) {
     ...
     */
    generateToolHandler(spec, messageType) {
        return async (args) => {
            try {
     ...
     */
    generateToolHandlerCode(spec, messageType) {
        const toolName = this.generateToolName(spec.protocol.name, messageType.name);
        const protocolName = spec.protocol.name;
...
`.trim();
     ...
     */
    countTools(spec) {
        return spec.messageTypes.filter(mt => mt.direction === 'request' || mt.direction === 'bidirectional').length;
    }
}
